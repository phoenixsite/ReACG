...
class Criterion:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """
        logger.info(f"criterion_name: {criterion_name}")
        _tmp_name_list = criterion_name.split("_")
        if _tmp_name_list[-1] == "guide":
            criterion_name = "_".join(_tmp_name_list[:-1])
        with torch.set_grad_enabled(enable_grad):
            x.requires_grad_(enable_grad)
            with torch.autocast(
                device_type="cuda", dtype=torch.float32, enabled=use_amp
            ):
                logit = self.model(x) / scale
            _scale = get_output_scale(
                logit.detach().clone(), 1.0, logit.device
            )  # 1.0は提案論文のデフォルト値
            x_sorted, ind_sorted = logit.sort(dim=1)
            ind = None if y is None else (ind_sorted[:, -1] == y).float()
            z_y = None if y is None else logit[torch.arange(logit.shape[0]), y]
            max_zi = (
                None
                if y is None
                else x_sorted[:, -2] * ind + x_sorted[:, -1] * (1.0 - ind)
            )

            if criterion_name == "mifpe":
                logit /= _scale
                loss = self.criterions["ce"](
                    logit,
                    y,
                    z_y=z_y,
                    max_zi=max_zi,
            ...
            elif criterion_name == "targeted_mifpe":
                logit /= _scale
                loss = self.criterions["targeted_ce"](
                    logit,
                    y,
                    z_y=z_y,
                    max_zi=max_zi,
            ...
            if original_logits is not None and _tmp_name_list[-1] == "guide":
                loss += max(0.0, _lambda - 2 * iteration) * (
                    logit - original_logits
                ).sum(1).pow(2)
        ...
        with torch.no_grad():
            target = (
                None
                if y is None
                else (ind * ind_sorted[:, -2] + (1 - ind) * ind_sorted[:, -1])
                .detach()
                .clone()
                .cpu()
            )
            cw_loss = (
                None
                if y is None
                else self.criterions["cw"](
                    logit, y, z_y=z_y, max_zi=max_zi, *args, **kwargs
                )
                .detach()
                .clone()
            )
            softmax_cw_loss = (
                None
                if y is None
                else self.criterions["softmax_cw"](
                    logit, y, z_y=z_y, max_zi=max_zi, *args, **kwargs
                )
                .detach()
                .clone()
            )
            acc = None if y is None else (y == logit.argmax(dim=1)).detach().clone()
        if grad is not None:
        ...
        if len(_criterion_name) == 1:
            ...
            if _criterion_name[0] == "scale":
                ...
                return self.criterion(
                    x, y, _criterion_name[-1], scale=scale, *args, **kwargs
                )
            else:
...
class CWLoss(UntargetedLoss):
    ...
    def __call__(self, logits, y_true, output_target_label=False, *args, **kwargs):
        return cw_loss(
            x=logits, y=y_true, output_target_label=output_target_label, *args, **kwargs
        )
...
class SumZLoss(UntargetedLoss):
    ...
    def __call__(
        self, logits, y_true, output_target_label=False, t=-3, *args, **kwargs
    ):
        return sum_z_loss(
            x=logits,
            y=y_true,
...
def sigmoid_cw_loss(
    x, y, z_y=None, max_zi=None, output_target_label=False, *args, **kwargs
):
    """
    Returns
    -------
